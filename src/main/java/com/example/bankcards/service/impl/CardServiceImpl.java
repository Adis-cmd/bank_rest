...
public class CardServiceImpl implements CardService {
    ...
    @Value("${card.max-transfer-amount:250000}")
    private BigDecimal maxTransferAmount;

    @Value("${card.card-expiration-years:3}")
    private Integer cardExpirationYears;


    @Override
    public Page<AdminCardResponse> getCardsByUserId(Long userId, Pageable pageable) {
        User user = userService.findById(userId);
        Page<Card> cards = cardRepository.findAllByOwnerId(user.getId(), pageable);

        return cards.map(card -> AdminCardResponse.builder()
...
@Override
public Page<AdminCardResponse> getAllCards(String status, Pageable pageable) {
...
private void validateTransfer(User user, Card fromCard, Card toCard, BigDecimal amount) {
    ...
    if (amount.compareTo(maxTransferAmount) > 0) {
        throw new TransferLimitExceededException("Сумма перевода превышает максимально допустимую: " + maxTransferAmount);
    }
...
public void createCard(CardCreateDto dto) {
    ...
    if (user.getAuthority().getName().equals("ROLE_ADMIN")) {
        throw new AccessDeniedException("Администратор не может иметь карты");
    }
    ...
    Card card = Card.builder()
            ...
            .expirationDate(LocalDate.now().plusYears(cardExpirationYears))
